...
class CropOnMarkers(ImagePreprocessor):
    def __init__(self, marker_ops, cwd):
        ...
        if "sheetToMarkerWidthRatio" in marker_ops:
            ...
            marker = ImageUtils.resize_util(  marker, config.dimensions.processing_width  / int(marker_ops["sheetToMarkerWidthRatio"]),
            )
    ...
    def getBestMatch(self, image_eroded_sub):
        ...
        for r0 in np.arange(self.marker_rescale_range[1],self.marker_rescale_range[0],-1 * descent_per_step,):  # reverse order
            s = float(r0 * 1 / 100)
            if s == 0.0:
                continue
